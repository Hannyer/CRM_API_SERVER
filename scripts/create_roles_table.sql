...
SELECT v.name, v.description, v.requires_license, v.status
FROM (VALUES
  ('Administrador', 'Acceso completo al sistema', false, true),
  ('Conductor', 'Transporte; requiere licencia vigente', true, true),
  ('Recepcionista', 'Atención en recepción y reservas', false, true),
  ('Operador', 'Operaciones del día a día', false, true),
  ('Guía', 'Guías de actividades turísticas', false, true)
) AS v(name, description, requires_license, status)
WHERE NOT EXISTS (
