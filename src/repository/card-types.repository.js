...
async function listCardTypes({ page = 1, limit = 50, status = null } = {}) {
  ...
  const countResult = await pool.query(
    `SELECT COUNT(*) as total FROM ops.card_type ${whereClause}`,
    params
  );
...
async function getCardTypeById(id) {
  const { rows } = await pool.query(
    `SELECT id, name, status, created_at as "createdAt", updated_at as "updatedAt" FROM ops.card_type WHERE id = $1::uuid`,
    [id]
  );
  return rows[0] || null;
}

async function createCardType({ name, status = true }) {
  const { rows } = await pool.query(
    `
...
async function updateCardType(id, { name, status }) {
  const updates = [];
  const params = [];
  let paramIndex = 1;
  ...
  if (!updates.length) return getCardTypeById(id);
